...
<script src="https://cdn.jsdelivr.net/npm/select2@4.1.0-beta.1/dist/js/select2.min.js"></script>
        ...
        <div class="row justify-content-center">
            ...
            <div class="col-md-8">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('store-city-from-company-create') }}">
                            @csrf                                                    
                                    ...
                                    <select name="country_id" class="form-control livesearch" aria-label="Default select example">
                                        @if (session()->has('country_name') && session()->has('country_id'))
                                            <option value="{{ session('country_id') }}" >                                                                                            
                                                {{ session('country_name') }} {{ "<a href='../../companies/create/cities/create/countries/".session('country_id')."/edit' class='btn btn-link btn-sm float-right'>Измена</a>" }}
                                            </option>
                                        @endif                                        
                                    </select>                                                                                                                                                            
                                    @error('country_id')
    ...
    <script type="text/javascript">        
        $('.livesearch').select2({
                ...
                processResults: function (data) {
                    return {
                        results: $.map(data, function (item) {
                            return {
                                text: item.name + " <a href='../../../companies/create/cities/create/countries/"+item.id+"/edit' class='btn btn-link btn-sm float-right'>Измена</a>",
                                id: item.id
                            }
                        })
                    };
            ...
            "language": {
                "noResults": function(){
                    return "Држава није пронађена <a href='../../../companies/create/cities/create/countries/create' class='btn btn-primary btn-sm'>Креирајте нову</a>";                
                }
            },
