...
<script src="https://cdn.jsdelivr.net/npm/select2@4.1.0-beta.1/dist/js/select2.min.js"></script>
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="{{ route('companies.edit', $company->id) }}">{{ $company->name }}</a></li>                
            ...
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header">
                        ...
                        <a href="{{ route('companies.edit', ['company' => $company->id]) }}" class="float-right"><i class="fas fa-angle-left"></i><span class="px-2">Назад</span></a>                        
                    </div>
    
                    <div class="card-body">
                        <form method="POST" action="{{ route('update-city-from-company-edit', [$company->id, $city->id]) }}">
                            @csrf
                            @method('PUT')                            
                                    ...
                                    <select name="country_id" class="form-control livesearch" aria-label="Default select example">
                                        @if (session()->has('country_name') && session()->has('country_id'))
                                            <option value="{{ session('country_id') }}" >                                                                                            
                                                {{ session('country_name') }} {{ "<a href='../../../../../companies/".$company->id."/edit/cities/".$city->id."/edit/".session('country_id')."/edit' class='btn btn-link btn-sm float-right'>Измена</a>" }}                                                
                                            </option>
                                        @else
                                            <option value="{{ $city->country_id }}" >                                                                                                                                            
                                                {{ $country_name }} {{ "<a href='../../../../../companies/".$company->id."/edit/cities/".$city->id."/edit/countries/".$city->country_id."/edit' class='btn btn-link btn-sm float-right'>Измена</a>" }}
                                            </option>    
                                        @endif
                                    </select>                                                                                                                                                            
                                    @error('country_id')
    ...
    <script type="text/javascript">
        ...
        $('.livesearch').select2({
                ...
                processResults: function (data) {
                    return {
                        results: $.map(data, function (item) {
                            return {
                                text: item.name + " <a href='../../../../../../companies/"+company_id+"/edit/cities/"+city_id+"/edit/countries/"+item.id+"/edit' class='btn btn-link btn-sm float-right'>Измена</a>",
                                id: item.id
                            }
                        })
                    };
            ...
            "language": {
            "noResults": function(){
                return "Држава није пронађена <a href='../../../../../../companies/"+company_id+"/edit/cities/"+city_id+"/edit/countries/create' class='btn btn-primary'>Креирајте нову</a>";                
            }
        },
