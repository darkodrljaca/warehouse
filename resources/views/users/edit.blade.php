            ...
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header">
                        {{ __('Ажурирање корисника') }}
                        <a href="{{ route('users.index') }}" class="float-right">Назад</a>   
                    </div>
                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('users.update', $user->id) }}">
                            ...
                            <div class="form-group row">
                                <label for="first_name" class="col-md-4 col-form-label text-md-right">{{ __('Име') }}</label>
                            ...
                            <div class="form-group row">
                                <label for="last_name" class="col-md-4 col-form-label text-md-right">{{ __('Презиме') }}</label>
                            ...
                            <div class="form-group row">
                                <label for="email" class="col-md-4 col-form-label text-md-right">{{ __('Ел. Пошта') }}</label>
                ...
                <div class="m-2 p-2">
                    <form method="POST" action="{{ route('users.destroy', $user->id) }}">
                        @csrf
                        @method('DELETE')
                        <button class="btn btn-danger">Обриши {{ $user->username }}</button>
                    </form>
                </div>
            </div>
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header">{{ __('Промена лозинке') }}</div>
                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('users.change.password', $user->id) }}">
                            ...
                            <div class="form-group row">
                                <label for="password-confirm" class="col-md-4 col-form-label text-md-right">{{ __('Потврда лозинке') }}</label>
